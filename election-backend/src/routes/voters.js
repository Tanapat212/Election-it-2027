...
// สุ่มรหัสผ่าน 5 ตัวอักษร จากชุดตัวอักษรที่ตัดตัวที่สับสนกันง่ายออกแล้ว
// (ไม่มี 0/O, 1/I/l, เพราะพิมพ์แจกกระดาษแล้วเด็กมักอ่าน/พิมพ์ผิดระหว่างตัวเหล่านี้)
const SAFE_PASSWORD_CHARS = 'ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnpqrstuvwxyz23456789';
function genPassword(length = 5) {
  let out = '';
  for (let i = 0; i < length; i++) {
    out += SAFE_PASSWORD_CHARS[Math.floor(Math.random() * SAFE_PASSWORD_CHARS.length)];
  }
  return out;
}
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      for (const v of voters) {
        ...
        if (!isValidStudentId(v.student_id)) {
          skipped += 1;
          continue;
        }
      ...
      return reply.send({
        message: `นำเข้าสำเร็จ ${inserted} รายการ (ข้าม ${skipped} รายการ: ซ้ำ/ข้อมูลไม่ครบ/รหัสนักศึกษาไม่ใช่ตัวเลข 11 หลัก)`,
        inserted,
        skipped,
        credentials,
        stats,
