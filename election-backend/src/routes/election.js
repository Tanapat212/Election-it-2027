...
import {
  ...
  updateEventTitle, pauseVoting, resumeVoting, computeRemainingSeconds,
...
export default async function electionRoutes(fastify) {
  fastify.get('/api/election/state', async (request, reply) => {
    ...
    return reply.send({
      ...
      votingDeadlineAt: state.voting_deadline_at,
      votingDurationSeconds: state.voting_duration_seconds,
      isPaused: !!state.is_paused,
      remainingSeconds: computeRemainingSeconds(state), // คำนวณฝั่ง server กันนาฬิกาเครื่องผู้ใช้เพี้ยน
  ...
  fastify.post('/api/admin/election/open', { preHandler: fastify.requireAdmin }, async (request, reply) => {
    const { duration_minutes } = request.body || {};
    const durationMinutes = Number(duration_minutes) > 0 ? Number(duration_minutes) : null;
    const state = await openVoting(durationMinutes);
    await writeAuditLog(fastify, request, 'open_voting', 'election_state', state.id, { duration_minutes: durationMinutes });
    fastify.io.emit('election:state', {
      votingStatus: state.voting_status,
      votingDeadlineAt: state.voting_deadline_at,
      votingDurationSeconds: state.voting_duration_seconds,
      isPaused: false,
      remainingSeconds: computeRemainingSeconds(state),
    });
    return reply.send({ message: 'เปิดระบบการโหวตแล้ว', state });
  });

  // หยุดนาฬิกานับถอยหลังชั่วคราว (เช่น ระบบล่มกลางคัน) — นักเรียนยังโหวตต่อได้ปกติ แค่เวลาหยุดนับ
  fastify.post('/api/admin/election/pause', { preHandler: fastify.requireAdmin }, async (request, reply) => {
    let state;
    try {
      state = await pauseVoting();
    } catch (err) {
      return reply.code(err.statusCode || 400).send({ error: 'bad_request', message: err.message });
    }
    await writeAuditLog(fastify, request, 'pause_voting', 'election_state', state.id, {});
    fastify.io.emit('election:state', {
      isPaused: true,
      remainingSeconds: computeRemainingSeconds(state),
    });
    return reply.send({ message: 'หยุดนาฬิกานับถอยหลังชั่วคราวแล้ว', state });
  });

  // เล่นนาฬิกาต่อ — ระบบจะชดเชยเวลาที่หยุดไปกลับเข้า deadline ให้อัตโนมัติ
  fastify.post('/api/admin/election/resume', { preHandler: fastify.requireAdmin }, async (request, reply) => {
    let state;
    try {
      state = await resumeVoting();
    } catch (err) {
      return reply.code(err.statusCode || 400).send({ error: 'bad_request', message: err.message });
    }
    await writeAuditLog(fastify, request, 'resume_voting', 'election_state', state.id, {});
    fastify.io.emit('election:state', {
      isPaused: false,
      votingDeadlineAt: state.voting_deadline_at,
      remainingSeconds: computeRemainingSeconds(state),
    });
    return reply.send({ message: 'เล่นนาฬิกาต่อแล้ว (ชดเชยเวลาที่หยุดไปให้แล้ว)', state });
  });
