...
export default async function authRoutes(fastify) {
  // Admin / กรรมการ login
  // นับ limit ตาม "ชื่อผู้ใช้" แทนที่จะนับตาม IP — เพราะกรรมการหลายคนมักใช้ WiFi
  // วงเดียวกัน (IP เดียวกัน) ถ้านับตาม IP คนอื่นพิมพ์ผิดจะพลอยทำให้ทุกคนโดนบล็อกไปด้วย
  // นับตาม username แทน แต่ละบัญชีจะมีโควตาของตัวเอง ไม่กระทบกัน
  // (hook: 'preHandler' จำเป็น เพราะต้องรอ body ถูก parse ก่อนถึงจะอ่าน username ได้)
  fastify.post(
    '/api/auth/admin/login',
    {
      config: {
        rateLimit: {
          ...
          hook: 'preHandler',
          keyGenerator: (request) => (request.body && request.body.username)
            ? `admin-login:${request.body.username}`
            : request.ip,
        },
      },
    },
  ...
  // นับ limit ตาม "ชื่อผู้ใช้" แทน IP ด้วยเหตุผลเดียวกัน — นักเรียนทั้งห้อง/ทั้งโรงเรียน
  // มักใช้ WiFi วงเดียวกัน นับตาม username ทำให้แต่ละคนมีโควตาของตัวเอง ไม่ล็อกกันเป็นกลุ่ม
  fastify.post(
    '/api/auth/voter/login',
    {
      config: {
        rateLimit: {
          ...
          hook: 'preHandler',
          keyGenerator: (request) => (request.body && request.body.username)
            ? `voter-login:${request.body.username}`
            : request.ip,
        },
      },
    },
