...
npm run build
```
⚠️ **สำคัญ**: ขั้นตอน `npm run build` ของ frontend ต้องให้เซิร์ฟเวอร์ต่ออินเทอร์เน็ตออกไปหา
`fonts.googleapis.com` และ `fonts.gstatic.com` ได้ (ใช้ดาวน์โหลดฟอนต์ Sarabun มาเก็บไว้ในเครื่อง
ตอน build ครั้งเดียว หลังจากนั้นเว็บทำงานแบบ offline ได้ปกติ) ถ้า VPS อยู่หลังไฟร์วอลล์ที่ปิดกั้น
โดเมนสองตัวนี้ คำสั่ง build จะ error ทันที ให้เปิด outbound ให้สองโดเมนนี้ก่อน แล้วค่อยรัน build ใหม่
