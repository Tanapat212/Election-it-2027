...
// แยกเป็น component ย่อยเพื่อเรียก hook ได้อย่างถูกต้อง (เรียก hook ใน .map() ตรงๆ ไม่ได้)
function CandidateThumb({ photoPath }) {
  const url = useAuthedImage(photoPath);
  if (!photoPath) return '—';
  if (!url) return <div style={{ width: 40, height: 40, borderRadius: 8, background: 'var(--it-blue-light)' }} />;
  return <img src={url} alt="" width={40} height={40} style={{ objectFit: 'cover', borderRadius: 8 }} />;
}
...
export default function CandidatesSection({ token }) {
  ...
  async function submitAdd(e) {
    ...
    if (!/^\d{11}$/.test(addForm.student_id.trim())) {
      showToast('รหัสนักศึกษาต้องเป็นตัวเลข 11 หลักเท่านั้น', 'danger');
      return;
    }
    setAddBusy(true);
    try {
          ...
          <table className="table table-hover mb-0 align-middle">
            ...
            <tbody>
              {candidates.map((c) => (
                <tr key={c.id}>
                  <td>
                    <CandidateThumb photoPath={c.photo_path} />
                  </td>
            ...
            <div className="row g-2">
              ...
              <div className="col-6">
                ...
                <input className="form-control" required value={addForm.student_id}
                  ...
                  maxLength={11} inputMode="numeric" placeholder="ตัวเลข 11 หลัก" />
              </div>
              <div className="col-6">
