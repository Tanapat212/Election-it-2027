...
export default function VotersSection({ token }) {
  ...
  async function handleAdd(e) {
    ...
    if (!/^\d{11}$/.test(form.student_id.trim())) {
  ...
  function downloadSampleCsv() {
    const csv = 'student_id,full_name,program\n64010001001,สมชาย ใจดี,ปวช.3 เทคโนโลยีสารสนเทศ\n64010001002,สมหญิง รักเรียน,ปวส.1 เทคโนโลยีสารสนเทศ\n';
            ...
            <div className="mb-3 p-2 rounded" style={{ background: '#f0f7ff' }}>
              ...
              {pwMode === 'custom' && (
                <>
                  ...
                  {customPw && <PasswordStrengthMeter password={customPw} />}
                </>
              )}
            </div>
            ...
            <div className="modal-content it-card">
              ...
              <div className="modal-body">
                <form onSubmit={submitReset}>
                  ...
                  {resetPwMode === 'custom' && (
                    <input className="form-control form-control-sm mb-1" placeholder="อย่างน้อย 6 ตัวอักษร"
                      value={resetCustomPw} onChange={(e) => setResetCustomPw(e.target.value)} />
                    {resetCustomPw && <div className="mb-3"><PasswordStrengthMeter password={resetCustomPw} /></div>}
                  )}
