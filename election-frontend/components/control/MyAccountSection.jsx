      ...
      <div className="it-card p-4" style={{ maxWidth: 480 }}>
        ...
        <form onSubmit={submit}>
          <div className="mb-2">
            ...
            <PasswordInput className="form-control" required
              value={form.current_password}
              onChange={(e) => setForm({ ...form, current_password: e.target.value })} />
          </div>
          <div className="mb-2">
            <label className="form-label small">รหัสผ่านใหม่</label>
            <PasswordInput className="form-control" required minLength={6}
              value={form.new_password}
              onChange={(e) => setForm({ ...form, new_password: e.target.value })} />
            {form.new_password && <PasswordStrengthMeter password={form.new_password} />}
          </div>
          <div className="mb-3">
