...
export default function WinnerAnnouncementScene({ results, candidatesMeta, eventTitle }) {
  const winner = candidatesMeta?.find((c) => c.candidate_number === results?.winner?.candidateNumber);
  const photoUrl = useAuthedImage(winner?.photo_path || null);
      ...
      <div className="overlay-root position-relative" style={{ background: 'linear-gradient(135deg, #4338ca, #7c3aed, #be185d)' }}>
        ...
        {eventTitle && (
          <motion.div
            ...
            className="fw-bold text-white position-absolute top-0 start-50 translate-middle-x mt-4 text-center"
            style={{ fontSize: 'clamp(0.9rem, 1.8vw, 1.2rem)', textShadow: '0 4px 16px rgba(0,0,0,0.35)', maxWidth: '80vw', zIndex: 1 }}
          >
            {eventTitle}
          </motion.div>
        )}
