...
import { saveVoterAuth } from '@/lib/auth';
...
export default function VoterLoginPage() {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true);
    try {
      const data = await apiFetch('/api/auth/voter/login', { method: 'POST', body: { username, password } });
      saveVoterAuth(data.token, data.voter);
      showToast(`ยินดีต้อนรับ ${data.voter.fullName}`, 'success');
      router.push('/vote');
    } catch (err) {
    ...
    <>
      ...
      <div className="container py-5 d-flex justify-content-center">
        <div className="it-card p-4 it-fade-in" style={{ maxWidth: 420, width: '100%' }}>
          ...
          <p className="text-secondary text-center small mb-4">ใช้ชื่อผู้ใช้/รหัสผ่านที่ได้รับจากผู้ดูแลระบบ</p>
          <form onSubmit={handleSubmit} noValidate>
            <div className="mb-3">
              <label className="form-label">ชื่อผู้ใช้ (รหัสนักศึกษา)</label>
